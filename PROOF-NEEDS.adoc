...
* Prove: taint propagation is sound (no tainted value reaches a sink
...
* Prove: analysis does not have false negatives for the declared
...
* Prove: parser accepts valid PHP and rejects invalid PHP (or at
...
* Prove: sanitization transforms eliminate the security vulnerabilities
they claim to fix

==== WordPress-Specific Rules (src/Sanctify/WordPress/)

* `+Constraints.hs+`, `+Hooks.hs+`, `+Security.hs+`
* Prove: WordPress hook analysis correctly models WordPress execution
order

=== Recommended Prover

* *Agda* or *Lean4* — Haskell analysis tools have a strong tradition of
formal verification
* *Idris2* for ABI contracts

=== Priority

*HIGH* — Security analysis tool. If the taint analysis is unsound, users
trust code that is actually vulnerable. False negatives in a security
tool are worse than no tool at all.
