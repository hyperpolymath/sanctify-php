...
1. *wp-sinople-theme* - Semantic theme with IndieWeb/Micropub support 2.
*Zotpress* - Mature WordPress plugin (couldn’t run sanctify-php at all)

*Feedback Date*: 2025-12-27 *Current Version*: 0.1.0.0
...
____
*sanctify-php could not be run at all* because GHC (Haskell compiler)
was not available. Manual analysis was performed instead using
documented patterns.
____

This confirms the #1 adoption blocker: *the Haskell build requirement
prevents any usage*.
...
|No `+composer require+` install |*Critical* |PHP devs expect Composer
installation
...
____
*The Haskell dependency is a BLOCKER, not just an inconvenience.* In the
Zotpress integration, the tool could not be used at all. PHP developers
cannot and will not install GHC. *Pre-built binaries are not optional —
they are required for any adoption.*
____
...
*Implementation*: - [ ] Add GitHub Actions workflow for
cross-compilation - [ ] Use static linking
(`+-optl-static -optl-pthread+`) - [ ] Sign binaries (GPG + sigstore) -
...
PHP developers expect `+composer require+`. Provide a Composer plugin
that: 1. Detects platform (OS/arch) 2. Downloads the appropriate
pre-built binary 3. Provides Composer scripts integration
...
[arabic]
. *Distribution*: CI/CD for binary releases
. *Parser*: PHP 8.1+ enum support
. *Semantic*: RDF/Turtle detection and escaping rules
. *Documentation*: WordPress integration examples
