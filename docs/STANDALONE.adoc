...
This document defines what each tool must provide independently and the
enhanced capabilities when used together.

'''''

=== Philosophy

Each tool should be *fully functional standalone*. Integration provides
*enhanced capabilities*, not basic functionality.
...
[arabic]
. *Use WordPress functions for escaping*:
+
[source,php]
----
...
----
. *Warn about missing semantic escaping*:
+
....
...
sanctify-php detects Turtle/JSON-LD output and suggests correct Aegis
escaping:
...
* [ ] Warning mode: alert when semantic escaping needed but Aegis
missing
...
* [ ] Extended validators: `+int()+`, `+ip()+`, `+domain()+`,
`+uuid()+`, `+slug()+`
* [ ] Turtle escaping: `+Semantic\Turtle::escapeString()+`,
`+escapeIRI()+`
