...
This document clarifies when to use sanctify-php and php-aegis based on
...
[width="100%",cols="46%,41%,13%",options="header",]
|===
|Your Situation |Use php-aegis? |Why
|WordPress plugin/theme |⚠️ *Only for unique features* |WP core has
equivalent security
...
|Semantic web (RDF/Turtle) |✅ *Yes - unique value* |No other library
handles this

|IndieWeb (Micropub, etc.) |✅ *Yes - unique value* |Protocol-specific
security

|ActivityPub/Fediverse |✅ *Yes - unique value* |Content policy
enforcement
|===
...
. *Already Well-Secured WordPress Plugins*
...
----
. *Semantic Web Applications* (UNIQUE VALUE)
+
[source,php]
----
...
----
. *IndieWeb Applications* (UNIQUE VALUE)
+
[source,php]
----
...
----
. *ActivityPub/Fediverse* (UNIQUE VALUE)
+
[source,php]
----
...
[arabic]
. *WordPress Plugins/Themes (for standard security)*
+
WordPress already provides: | Need | WordPress Function | php-aegis Adds
Nothing | |——|——————-|———————-| | HTML escape | `+esc_html()+` | ❌ | |
Attribute escape | `+esc_attr()+` | ❌ | | URL escape | `+esc_url()+` |
❌ | | JS escape | `+esc_js()+` | ❌ | | Email validation |
`+is_email()+` | ❌ | | Sanitize text | `+sanitize_text_field()+` | ❌ |
+
*Exception*: Use php-aegis for Turtle/RDF/IndieWeb in WordPress themes.
. *PHP 7.4 Environments (until compat package exists)*
...
[width="100%",cols="27%,27%,46%",options="header",]
|===
...
[arabic]
. *Don’t compete with WordPress core* — document that php-aegis is for
non-WP apps or unique WP needs
. *Focus on unique value* — Turtle, IndieWeb, ActivityPub are unserved
markets
. *Create framework adapters* — Laravel, Symfony, WordPress (for unique
features only)

==== For sanctify-php Team

[arabic]
. *Pre-built binaries are MANDATORY* — tool literally cannot be used
without them
. *Docker image as fallback* — for environments that can’t install
binaries
. *Document when NOT to use* — mature WP plugins may not benefit
